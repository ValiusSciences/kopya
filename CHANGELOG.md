...
- **Documented high ambient RNA as a distinct failure mode.** Ambient RNA is
  ...
  against the diploid pool, so it attenuates real deviations by roughly `(1 - rho)`
  rather than creating false ones. The README now says how to recognise it from
  `qc.json` alone and when correction is and is not worth running. On a 12-sample
  internal cohort (SoupX `rho` 0.033 to 0.737) correction moved median AUC by +0.003
  and improved matched-bulk concordance in 3 of 11 scorable samples; only the one
