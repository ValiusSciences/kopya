...
- `--non-malignant-labels A,B,C`: override just the allow-list of cell types treated as "normal", without authoring a whole JSON. Add back a label the defaults exclude (e.g. `Fibroblast` on a sample whose normal stroma is genuinely fibroblast-rich) or drop one your tumor happens to score on. Validated against the active signature set, so a typo fails fast.
- `--gene-order path.tsv`: overrides the bundled GENCODE table (4 columns: `gene_symbol chr start end`).

> **The bundled signatures and allow-list are defaults, not universal truth.** They're curated for immune-infiltrated solid tumors. On mesenchymal (sarcoma/osteosarcoma) or hematologic samples the defaults can mislabel the malignant population as "normal": `Plasma_cell` is intentionally **excluded** from the allow-list, for instance, because plasma cells are the malignancy in myeloma, and `Fibroblast` is excluded because mesenchymal / ECM-like tumor cells score on the fibroblast program and would otherwise seed their own baseline. Both remain defined as signatures, so such cells still resolve to them and are kept out of the seed rather than falling through to a label that is allow-listed. If a run looks off (see [When to use supervised mode](#when-to-use-supervised-mode)), retune with `--non-malignant-labels` / `--signatures`, or pin the baseline outright with `--norm-cell-names`. The effective allow-list is recorded in `qc.json` (`baseline_diagnostics.non_malignant_labels`).
...
**On `uncertain` (flag, don't force).** Three gates downgrade untrustworthy calls to `uncertain` rather than asserting them. Two act on `tumor` calls: the **low-complexity** gate (above) and a **coherence** gate, where a high score built from scattered, single-segment spikes rather than contiguous chromosome-arm-scale runs is more likely a transcriptionally-"loud" normal cell than copy number. The third acts on `normal` calls: the **anti-alignment** gate, where a cell carrying a large, coherent deviation pointing *against* the consensus profile is not a diploid cell even though the signed score puts it next to one — most often a transcriptional outlier, occasionally a subclone whose profile opposes the one the template locked onto (see *Known limitations* in `CHANGELOG.md`). All three gates are *subtractive* — none ever promotes a cell to tumor — so they cannot inflate the tumor set. Genuinely ambiguous cells, including same-lineage normals whose expression mimics CNV (e.g. normal astrocytes vs AC-like glioma, which expression-only CNV cannot separate), land in `uncertain` by design. `n_uncertain`, `n_low_complexity` and `n_anti_aligned` are recorded in `qc.json`. The anti-alignment gate's thresholds are set relative to the malignant population's own score and burden, so it **declines to run at all when fewer than 30 cells are called tumor** — below that the median it needs is one or two cells and the gate would move with their noise. `anti_alignment_tumor_n` in `qc.json` reports how many tumor calls the estimate rested on, and is `0` when the gate was skipped.
